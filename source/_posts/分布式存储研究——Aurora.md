...
+ Multi-tenancy and Database Consolidation：许多客户的服务会面临多租户的场景，他们原有的应用不容易拆分，那么简单的方法就是为每个租户维持一张表，并把所有租户的表放到数据库中去。这和类似于Salesforce使用多租户数据结构，并把所有用户数据封装到一个表里的行为不一样。这种场景下，需要考虑一下几个方面，Aurora都非常适合。
